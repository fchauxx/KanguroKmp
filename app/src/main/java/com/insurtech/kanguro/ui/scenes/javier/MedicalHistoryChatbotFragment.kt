...
class MedicalHistoryChatbotFragment :
    ...
    private val args: MedicalHistoryChatbotFragmentArgs by navArgs()

    override val screenName: AnalyticsEnums.Screen = AnalyticsEnums.Screen.ReviewDocumentsChatBot
    ...
    private fun navigateToMap() {
        findNavController().safeNavigate(MedicalHistoryChatbotFragmentDirections.actionGlobalChatBotMapFragment())
    }
    ...
    private fun handleLocalActionSelected(action: ChatAction) {
        when (action.value) {
            ...
            R.string.vet_advice.toString() -> {
                findNavController().safeNavigate(MedicalHistoryChatbotFragmentDirections.actionGlobalVetAdviceFragment())
            }

            R.string.frequent_questions.toString() -> {
                findNavController().safeNavigate(
                    MedicalHistoryChatbotFragmentDirections.actionGlobalInformationTopicsFragment(
    ...
    private fun handleOtpValidation() {
        findNavController().safeNavigate(
            MedicalHistoryChatbotFragmentDirections.actionMedicalHistoryChatbotFragmentToCodeValidationFragment()
        )
        ...
        when (type) {
            ...
            is ChatbotType.NewClaim -> {
                with(findNavController()) {
                    // Remove the chatbot screen, stacks the claims screen and shows the feedback fragment.
                    popBackStack()
                    safeNavigate(MedicalHistoryChatbotFragmentDirections.actionGlobalPetTrackYourClaimsFragment())
                    viewModel.getSubmittedClaimId()?.let { claimId ->
                        safeNavigate(
                            MedicalHistoryChatbotFragmentDirections.actionGlobalExperienceFeedbackFragment(
                                claimId
                            )
                        )
                    }
